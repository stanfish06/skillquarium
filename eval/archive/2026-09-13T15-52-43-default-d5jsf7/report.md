...
skill%  = traits this skill prescribes. full%  = whole rubric, incl. blind traits no skill asks for.
Percentages average only gate-passing cells. gate = passed / reached the gate.
trunc = hit the output cap. err = gateway or tool failure; both shrink the sample.

```
model                  task                 skill                  arm      n   gate   trunc err skill%  full%   think
---------------------- -------------------- ---------------------- -------- --- ------ ----- --- ------- ------- ------
...
## bench (median over gate-passing cells)

```
model                  task                 skill                  arm          ns/op  B/op  allocs failed
...
```
ns/op is machine noise across runs; compare arms on B/op and allocs/op. allocs is `-` where the runtime cannot count (.NET).
