...
Writes the canonical bundle into ``<output_dir>/reproducibility/``:
    ...
    checksums.sha256               (generated — outputs minus work/.nextflow/reproducibility/logs and the root report.md/result.json)
...
# ``reproducibility`` carries the bundle itself; ``logs`` holds execution logs
# (kept out of the manifest, as in nfcore-rnaseq/scrnaseq).
_CHECKSUM_EXCLUDE_DIRS: tuple[str, ...] = ("work", ".nextflow", "reproducibility", "logs")
_CHECKSUM_EXCLUDE_SUFFIXES: tuple[str, ...] = (".log",)
# Root-level run summaries (generated by reporting.py / --check, not pipeline
# outputs): excluded from the manifest, matching the sibling wrappers.
# ``check_result.json`` may linger at the root when a --check precedes a real run
# in the same directory, so it is kept out of the manifest too.
_CHECKSUM_EXCLUDE_ROOT_FILES: frozenset[str] = frozenset(
    {"report.md", "result.json", "check_result.json"}
)
