...
> Prerequisite: the Agent Mode below reads five researcher prompts at `../../agents/` (`crate-researcher.md`, `rust-changelog.md`, `std-docs-researcher.md`, `docs-researcher.md`, `clippy-researcher.md`), which are not vendored in this flat vault — they live at the plugin root upstream. Install the rust-skills plugin from https://github.com/actionbook/rust-skills so the relative paths resolve. Standalone, the Execution Mode Detection check fails and the skill runs its Inline Mode.
