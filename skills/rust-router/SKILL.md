...
> Prerequisite: `patterns/negotiation.md` points to `_meta/error-protocol.md`, `_meta/negotiation-protocol.md`, `_meta/negotiation-templates.md` and `agents/_negotiation/response-format.md`, which are not vendored in this flat vault — they live at the plugin root upstream. Install the rust-skills plugin from https://github.com/actionbook/rust-skills to read them. The routing tables in this file and in `patterns/` are self-contained; only those four cross-references dangle.
