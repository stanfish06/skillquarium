...
> Prerequisite: the Agent Mode below needs the upstream `../../agents/` directory and the `/create-llms-for-skills` and `/create-skills-via-llms` commands from `commands/`, none of which are vendored in this flat vault — they live at the plugin root upstream. Install the rust-skills plugin from https://github.com/actionbook/rust-skills so they resolve. Standalone, the Execution Mode Detection check fails and the skill runs its Inline Mode.
