---
name: mermaid-studio
description: Validate and render Mermaid diagrams to SVG, PNG, or ASCII with bundled scripts (mermaid-cli, beautiful-mermaid, Puppeteer icon packs), plus styling rules for C4 and AWS architecture-beta diagrams. Use when a .mmd file needs a syntax check, a rendered image, batch rendering, ASCII output, an AWS icon diagram, a C4 diagram, or one of the 20 bundled themes. Do not use for choosing a diagram type or writing Mermaid inside markdown docs (use markdown-mermaid-writing or diagram-generator), or for previewing in the terminal (use mermaid-terminal).
license: CC-BY-4.0
metadata:
...
```bash
node ~/.agents/skills/mermaid-studio/scripts/validate.mjs <file.mmd>
```
...
```bash
bash ~/.agents/skills/mermaid-studio/scripts/setup.sh
```

Installs mermaid-cli, beautiful-mermaid, mermaid, puppeteer, and Chromium into `${XDG_CACHE_HOME:-~/.cache}/mermaid-studio` (override with `MERMAID_STUDIO_DIR`). Run once per environment.

### 4.2 — Single Diagram Rendering

```bash
# SVG (default — best quality)
node ~/.agents/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.svg

# PNG (for documents/presentations)
node ~/.agents/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.png --width 1200

# ASCII (for terminals/READMEs)
node ~/.agents/skills/mermaid-studio/scripts/render-ascii.mjs -i diagram.mmd

# With icon packs (architecture-beta with AWS/tech icons)
node ~/.agents/skills/mermaid-studio/scripts/render.mjs -i diagram.mmd -o diagram.svg --icons logos,fa
```
...
```bash
node ~/.agents/skills/mermaid-studio/scripts/batch.mjs \
  --input-dir ./diagrams \
  --output-dir ./rendered \
  --format svg \
  --theme default \
