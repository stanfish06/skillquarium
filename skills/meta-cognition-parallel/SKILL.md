...
> Prerequisite: the Agent Mode below reads `../../agents/layer1-analyzer.md`, `layer2-analyzer.md` and `layer3-analyzer.md`, which are not vendored in this flat vault — they live at the plugin root upstream. Install the rust-skills plugin from https://github.com/actionbook/rust-skills so the relative paths resolve. Standalone, the Execution Mode Detection check fails and the skill runs its sequential Inline Mode.
