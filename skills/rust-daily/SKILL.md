...
> Prerequisite: the Agent Mode below reads `../../agents/rust-daily-reporter.md`, which is not vendored in this flat vault — it lives at the plugin root upstream. Install the rust-skills plugin from https://github.com/actionbook/rust-skills so the relative path resolves. Standalone, the Execution Mode Detection check fails and the skill runs its Inline Mode, fetching each source directly.
