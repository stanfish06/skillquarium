    ...
    this.help.add(
      new TextRenderable(renderer, {
        ...
        content: [
          ...
          "b→s      without the skill → with the skill. Without: the task prompt alone. With: SKILL.md added to the system prompt",
          "Δskill   skill% with the skill minus skill% without it, in percentage points",
          "Δfull    same for full%",
          "gate     cells that compiled and passed the task's behaviour check / cells that reached the gate,",
          "         for each side. Only gate-passing cells count toward the percentages",
          ...
          "time per call         nanoseconds for one run of the task's benchmark body; depends on machine load",
          "bytes per call        heap bytes the code allocated during one run; stable across machines",
