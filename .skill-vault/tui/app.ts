...
export class SkillquariumApp {
  ...
  constructor(
    ...
    this.help.add(
      new TextRenderable(renderer, {
        ...
        width: "100%",
        content: [
          "skill%     percent of the traits specified in the skill that the code implements",
          "full%      percent of all specified traits that the code implements",
          "b→s        without the skill → with the skill",
          "Δskill     skill% with minus without, percentage points",
          "Δfull      same for full%",
          "compiled   compiled and passed tests / total, without | with",
          "trunc      hit the output token cap",
          "err        gateway or tool failure",
          "think      median reasoning tokens",
          "time       ns per call of the benchmark body, median",
          "bytes      heap bytes allocated per call, median",
          "allocs     heap allocations per call, median; none for .NET",
          "",
          "? or Esc closes",
        ].join("\n"),
      }),
  ...
  private updateBenchDetail(): void {
    ...
    const lines = [
      ...
      line("think", num(b.reasoning), s ? num(s.reasoning) : ""),
      line("tool calls", num(b.toolCalls), s ? num(s.toolCalls) : ""),
      "",
      line("time, ns", bench(b, "ns"), bench(s, "ns")),
      line("bytes", bench(b, "bytes"), bench(s, "bytes")),
      line("allocs", bench(b, "allocs"), bench(s, "allocs")),
